<x-app-layout>
    ...
    <div class="mb-6 flex justify-between items-center">
        <h3 class="text-lg font-bold text-gray-800 font-heading">Daftar Fasilitas</h3>
        <a href="{{ route('admin.facilities.create') }}" class="bg-primary hover:bg-secondary text-white font-medium py-2.5 px-5 rounded-xl transition-all duration-300 shadow-sm hover:shadow-md flex items-center gap-2">
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50/50">
                    <tr>
                        <th class="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Gambar & Nama</th>
                    ...
                    <tr class="hover:bg-gray-50/50 transition-colors">
                            ...
                            <div class="flex items-center">
                                <div class="flex-shrink-0 h-12 w-12 rounded-xl overflow-hidden bg-gray-100 border border-gray-200">
                                    @if($facility->image_path)
                                        <img class="h-12 w-12 object-cover" src="{{ Storage::url($facility->image_path) }}" alt="">
                                    @else
                                ...
                                <div class="ml-4">
                                    <div class="font-bold text-gray-900">{{ $facility->name }}</div>
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-4 text-sm text-gray-500">
                            <div class="max-w-xs truncate">{{ $facility->description ?? '-' }}</div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                            <div class="flex items-center gap-3">
                                <a href="{{ route('admin.facilities.edit', $facility) }}" class="text-blue-600 hover:text-blue-900 bg-blue-50 hover:bg-blue-100 p-2 rounded-lg transition-colors">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                </a>
                                <form action="{{ route('admin.facilities.destroy', $facility) }}" method="POST" class="inline-block" onsubmit="return confirm('Yakin ingin menghapus fasilitas ini?');">
                                    @csrf
                                    @method('DELETE')
                    ...
                    <tr>
                        <td colspan="4" class="px-6 py-12 text-center text-gray-500">
                            <div class="flex flex-col items-center justify-center">
                                ...
                                <span class="text-gray-400 font-medium">Belum ada data fasilitas.</span>
                            </div>
                        </td>
                    </tr>
                    @endforelse
