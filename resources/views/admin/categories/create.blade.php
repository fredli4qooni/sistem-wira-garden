<x-app-layout>
    ...
    <div class="mb-6 flex justify-between items-center">
        <h3 class="text-lg font-bold text-gray-800 font-heading">Tambah Kategori Baru</h3>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="p-6 md:p-8" x-data="{ iconType: 'svg', previewSvg: '' }">
            <form action="{{ route('admin.categories.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="space-y-6">
                    <div>
                        ...
                        <input type="text" name="name" id="name" required class="block w-full border-gray-200 rounded-xl shadow-sm focus:ring-secondary focus:border-secondary transition-colors py-3 px-4 bg-gray-50 focus:bg-white" value="{{ old('name') }}" placeholder="Contoh: Petualangan, Wahana Air">
                    ...
                    <div x-show="iconType === 'svg'" x-transition class="p-4 rounded-xl border border-gray-200 bg-gray-50">
                        ...
                        <div class="relative max-w-sm">
                            <input type="text" name="icon_svg" id="icon_picker" class="block w-full border-gray-200 rounded-xl shadow-sm focus:ring-secondary focus:border-secondary transition-colors py-3 px-4 pl-12 bg-white" placeholder="fas fa-tree" value="{{ old('icon_svg', 'fas fa-tree') }}" autocomplete="off">
                            <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                                <i class="fas fa-tree text-green-600 text-lg" id="icon_preview"></i>
                            </div>
                        </div>
                    ...
                    <div x-show="iconType === 'image'" x-transition style="display: none;" class="p-4 rounded-xl border border-gray-200 bg-gray-50">
                        ...
                        <p class="text-xs text-gray-500 mb-3">Format: JPG, PNG, WEBP, SVG. Maks 2MB. Disarankan rasio 1:1.</p>
                    ...
                    <div>
                        <label class="flex items-center gap-3 cursor-pointer">
                            <input type="checkbox" name="is_active" value="1" checked class="w-5 h-5 rounded border-gray-300 text-primary focus:ring-primary transition-colors">
                ...
                <div class="mt-8 pt-6 border-t border-gray-100 flex justify-end gap-3">
                    <button type="reset" class="px-6 py-2.5 rounded-xl border border-gray-300 text-gray-700 font-medium hover:bg-gray-50 transition-colors">Reset</button>
                    <button type="submit" class="px-6 py-2.5 rounded-xl bg-primary text-white font-medium hover:bg-secondary shadow-sm hover:shadow-md transition-all">Simpan Kategori</button>
                </div>
            </form>
        </div>
    </div>
